...
type IndelRatesArrayTests() =
    ...
    let idrA rates = indelRatesArray.create (Array.create 1 rates)
    let inserter () = 0
    let mutator x = x + 1
    let arrayToMutate = [| 1 |]
    ...
    let ``mutate should apply Mutation mode correctly`` () =
        let rates = idrA (createRates (1.0, 0.0, 0.0)) // Always Mutation
    ...
    let ``mutate should handle single-element array correctly`` () =
        let rates = idrA (createRates (0.0, 0.0, 1.0)) // Deletion
        let result = IndelRatesArray.mutate rates inserter mutator (fun () -> 0.0) arrayToMutate
