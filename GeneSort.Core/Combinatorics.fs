...
module Combinatorics =
    ...
    let cartesianProduct2d (seq_a: seq<'a>) (seq_b: seq<'b>) : ('a*'b) seq =
    ...
    let rec cartesianProductLists (lists: list<list<'a>>) : 'a list seq =
        match lists with
        | [] -> Seq.empty<'a list>
        | [single] ->
        ...
        | hd::tl ->
            seq { for x in hd do
                    for rest in cartesianProductLists tl do
                        yield x :: rest }


    //Produces the cartesian product of array1 and array2, where the elements
    //of array1 are all the same length, and the elements of array2 are all the same length.
    let cartesianProductOfArrays (array1: 'a[][]) (array2: 'a[][]) : 'a[][] =
        let len1 = array1.Length
        let len2 = array2.Length
        if len1 = 0 || len2 = 0 then
            [||]
        else
            let sublen1 = array1.[0].Length
            let sublen2 = array2.[0].Length
            let outlen = sublen1 + sublen2
            let result: 'a[][] = Array.zeroCreate (len1 * len2)
            for i = 0 to len1 - 1 do
                for j = 0 to len2 - 1 do
                    let idx = i * len2 + j
                    let arr = Array.zeroCreate outlen
                    Array.blit array1.[i] 0 arr 0 sublen1
                    Array.blit array2.[j] 0 arr sublen1 sublen2
                    result.[idx] <- arr
            result
