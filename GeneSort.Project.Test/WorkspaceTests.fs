...
type WorkspaceTests() =
    ...
    //    let run = run.create 3 cycle { Index = 3; Cycle = cycle; Parameters = Map.ofList [("algorithm", "quick")] }
    //    let filePath = OutputData.getOutputFileName workspace.WorkspaceFolder run.Index run.Cycle (outputDataType.Run |> OutputDataType.toString)
    //    let expectedPath = Path.Combine(tempDir, workspace.Name,  "Run", "Run_1_3.msgpack")
    ...
    //    let runsFolder = OutputData.getOutputDataFolder workspace outputDataType.Run
