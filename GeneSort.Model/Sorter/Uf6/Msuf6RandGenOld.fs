...
type Msuf6RandGenOld = 
    ...
    with
    /// Creates an Msuf6RandGenOld with the specified parameters.
    ...
    member this.toString() =
        let genRatesStr = 
            ...
            |> String.concat ", "
        sprintf "Msuf6RandGenOld(RngType=%A, Width=%d, StageCount=%d, GenRates=%s)" 
                this.RngType 
                (%this.SortingWidth) 
                (%this.StageCount)
                genRatesStr
    ...
    interface IEquatable<Msuf6RandGenOld> with
...
module Msuf6RandGenOld =

    /// Generates a unique ID for an Msuf6 instance based on the Msuf6RandGenOld configuration and an index.
    let makeId (msuf6RandGen: Msuf6RandGenOld) (index: int) : Guid<sorterModelID> =
        [ 
    ...
    /// Returns a string representation of the Msuf6RandGenOld configuration.
    let toString (msuf6Gen: Msuf6RandGenOld) : string =
        let genRatesStr = 
            msuf6Gen.GenRates 
            |> Array.mapi (fun i gr -> 
                sprintf "[%d: Ortho=%f, Para=%f, SelfRefl=%f]" 
    ...
    /// <param name="msuf6Gen">The Msuf6RandGenOld configuration (genRates ignored).</param>
