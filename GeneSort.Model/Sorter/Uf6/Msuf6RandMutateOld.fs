...
type Msuf6RandMutateOld = 
    ...
    static member createFromSingleRate
        ...
        Msuf6RandMutateOld.create rngType stageCount order 
    ...
    interface IEquatable<Msuf6RandMutateOld> with
        member this.Equals(other) = 
            this.rngType = other.rngType && 
            this.stageCount = other.stageCount &&
            this.probabilities = other.probabilities

module Msuf6RandMutateOld =

    /// Generates a unique ID for a mutated Msuf6 instance.
    let makeId (msuf6RandMutate: Msuf6RandMutateOld) (msuf6: Msuf6) (index: int) : Guid<sorterModelID> =
        [ 
    ...
    /// Returns a string representation of the Msuf6RandMutateOld configuration.
    let toString (msuf6RandMutate: Msuf6RandMutateOld) : string =
        let ratesStr = 
            ...
            |> String.concat ", "
        sprintf "Msuf6RandMutateOld(RngType=%A, StageCount=%d, MutationRates=%s)" 
                msuf6RandMutate.RngType 
                (%msuf6RandMutate.StageCount)
                ratesStr

    /// Mutates an Msuf6 instance using the specified Msuf6RandMutateOld configuration and random number generator.
    /// <param name="randoGen">The random number generator factory.</param>
    /// <param name="msuf6RandMutate">The Msuf6RandMutateOld configuration.</param>
    ...
    let mutate
            ...
            (msuf6RandMutate: Msuf6RandMutateOld)
            (msuf6: Msuf6) 
            (index: int) : Msuf6 =
        if msuf6.StageCount <> %msuf6RandMutate.StageCount then
            failwith $"Stage count of Msuf6 {%msuf6.StageCount} must match Msuf6RandMutateOld {%msuf6RandMutate.StageCount}"
