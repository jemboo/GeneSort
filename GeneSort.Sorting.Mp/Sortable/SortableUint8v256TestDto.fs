...
module SortableUint8v256TestDto =
    ...
    let private blockFromDomain (block: simdSortBlock) : simdSortBlockDto =
        let raw = 
            block.Vectors 
            |> Array.map (fun v -> 
    ...
    let private blockToDomain (dto: simdSortBlockDto) : simdSortBlock =
        let vecs = 
            dto.RawVectors 
            |> Array.map (fun bytes -> Vector256.Create<byte>(bytes))
        
        simdSortBlock.createFromVectors vecs dto.SortableCount
