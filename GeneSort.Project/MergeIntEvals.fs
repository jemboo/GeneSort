...
module MergeIntEvals = 
    ...
    let binReportExecutor 
            ...
            try
                ...
                let reportFilePath = Path.Combine(project.ProjectFolder, sprintf "%s_SorterEvalReport_%s.txt" "SorterSetEvalSamples" (DateTime.Now.ToString("yyyyMMdd_HHmmss")))
    ...
    let ceUseProfileReportExecutor 
            ...
            try
                ...
                // Save the report to a file
                let reportFilePath = Path.Combine(project.ProjectFolder, sprintf "SorterCeUseReport_%s.txt" (DateTime.Now.ToString("yyyyMMdd_HHmmss")))
                File.WriteAllText(reportFilePath, reportContent)
    ...
    let RunSorterEvalReport() =
        let cts = new CancellationTokenSource()
        (binReportExecutor project.ProjectFolder cts progress)
        (ceUseProfileReportExecutor project.ProjectFolder cts progress)
