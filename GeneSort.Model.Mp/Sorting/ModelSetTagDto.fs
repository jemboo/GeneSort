...
module SplitJoinDto =
    let fromDomain (sj: splitJoin) : splitJoinDto =
        match sj with
    ...
    let toDomain (dto: splitJoinDto) : splitJoin =
        match dto with
...
module ModelTagDto =
    let fromDomain (tag: modelTag) : modelTagDto =
        match tag with
        | Single -> 
            { Tag = "Single"; SplitJoin = None }
        | SplitPair sj -> 
            { Tag = "SplitPair"
              SplitJoin = Some (SplitJoinDto.fromDomain sj) }
    
    let toDomain (dto: modelTagDto) : modelTag =
        match dto.Tag with
        | "Single" -> Single
        | "SplitPair" ->
            match dto.SplitJoin with
            | Some sj -> SplitPair (SplitJoinDto.toDomain sj)
...
module ModelSetTagDto =
    ...
    let fromDomain (tag: modelSetTag) : modelSetTagDto =
        { ParentId = (%tag.SortingId).ToString()
          ModelTag  = ModelTagDto.fromDomain tag.ModelTag }

    let toDomain (dto: modelSetTagDto) : modelSetTag =
        let parentId = Guid.Parse(dto.ParentId) |> UMX.tag<sortingId>
        let modelTag = ModelTagDto.toDomain dto.ModelTag
